...
class TestResticShell(TestCase):
    ...
    def test_restic_shell_single_repo(self, mock_sys_exit, mock_logger):
        ...
        with patch("builtins.print") as mock_print, patch("runrestic.restic.shell.pty.spawn") as mock_spawn:
            shell.restic_shell(configs)

            mock_print.assert_any_call("Using: \033[1;92mTestConfig:test_repo\033[0m")
            mock_print.assert_any_call("Spawning a new shell with the restic environment variables all set.")
    ...
    @patch("runrestic.restic.shell.sys.exit")
    def test_restic_shell_multi_repo_invalid_selection(self, mock_sys_exit, mock_input, mock_logger):
        """
        ...
        self.assertTrue(str(context.exception) == "Invalid selection. Please choose a valid repository index.")
